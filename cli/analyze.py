...
def analyze(repo_url, threshold, output, output_format, github_token):
    ...
    try:
        ...
        if not analyzer.proprietary_available:
            click.echo("⚠️  Running in demo mode - install repo-intelligence-core for full analysis", err=True)
        ...
        status = "✅ HEALTHY" if result.get("classification") == "HEALTHY" else "⚠️  NEEDS ATTENTION"
...
def _print_summary(result, output):
    ...
    if result.get('demo_mode'):
        click.echo("\n" + "!" * 50, file=output)
        click.echo("DEMO MODE - Install repo-intelligence-core for real analysis", file=output)
        click.echo("!" * 50, file=output)
